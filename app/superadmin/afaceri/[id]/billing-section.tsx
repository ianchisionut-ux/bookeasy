...
export default function BillingSection(p:Props){
 ...
 const [invoiceName,setInvoiceName]=useState(p.invoiceName)
 ...
 async function save(){setSaving(true);setMessage('');try{
  ...
  if(file){const form=new FormData();form.set('file',file);const upload=await fetchWithTimeout(`/api/superadmin/businesses/${p.businessId}/invoice`,{method:'POST',body:form},30000);const data=await upload.json().catch(()=>({}));if(!upload.ok)throw new Error(data.error||'Upload eșuat.');setInvoiceName(file.name);setFile(null)}
 ...
 async function removeInvoice(){if(!confirm('Ștergi factura încărcată? Clientul nu o va mai putea descărca.'))return;setSaving(true);setMessage('');try{const res=await fetchWithTimeout(`/api/superadmin/businesses/${p.businessId}/invoice`,{method:'DELETE'});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||'Ștergerea a eșuat.');setInvoiceName(null);setMessage('Factura a fost ștearsă.');router.refresh()}catch(e:any){setMessage(e?.message||'Ștergerea a eșuat.')}finally{setSaving(false)}}
 ...
 return <Card>
  ...
  <button type="button" onClick={()=>inputRef.current?.click()} onDragOver={e=>{e.preventDefault();setDragging(true)}} onDragLeave={()=>setDragging(false)} onDrop={dropped} className={`w-full rounded-2xl border-2 border-dashed p-5 text-center transition-colors ${dragging?'border-[var(--accent)] bg-[var(--accent-soft)]':'border-gray-200 bg-gray-50'}`}><span className="block text-sm font-medium">Trage factura aici sau apasă pentru upload</span><span className="block text-xs text-gray-400 mt-1">PDF, JPG sau PNG · maximum 10 MB</span>{(file||invoiceName)&&<span className="block text-xs text-[var(--accent)] mt-2">{file?`Fișier nou: ${file.name}`:`Factură curentă: ${invoiceName}`}</span>}</button>
  {invoiceName&&<div className="mt-2 flex items-center gap-2"><a href={`/api/billing/invoice/${p.businessId}`} className="text-xs text-[var(--accent)] hover:underline">Descarcă factura curentă</a><button type="button" onClick={removeInvoice} disabled={saving} title="Șterge factura" aria-label="Șterge factura" className="grid h-6 w-6 place-items-center rounded-full text-sm font-semibold text-red-600 hover:bg-red-50">×</button></div>}
