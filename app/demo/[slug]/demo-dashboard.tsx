...
function DemoButton({ children }: { children: React.ReactNode }) {
  return <button type="button" disabled title="Indisponibil în modul demonstrație" className="rounded-xl bg-[#14142b] px-4 py-2 text-sm font-medium text-white opacity-55 cursor-not-allowed">{children}</button>
}
  ...
  return (
    <div className="min-h-screen bg-[#f7f8f6] text-[#17172d]" style={{ '--demo-accent': accentColor } as React.CSSProperties}>
      ...
      <main className="min-h-screen p-4 sm:p-7 lg:ml-[220px] lg:p-8">
        ...
        {section === 'calendar' && <CalendarPreview accent={accentColor} />}
        {section === 'messages' && <MessagesPreview />}
        {section === 'bookings' && <BookingsPreview label={bookingLabel} />}
        {section === 'customers' && <CustomersPreview label={customerLabel} isClinic={isClinic} />}
        {section === 'services' && <ServicesPreview label={serviceLabel} isVenue={isVenue} />}
        {section === 'reviews' && <ReviewsPreview />}
        {section === 'stats' && <StatsPreview />}
        {section === 'settings' && <SettingsPreview name={businessName} teamSize={teamSize} />}
      </main>
    </div>
  )
}
...
function CalendarPreview({ accent }: { accent: string }) {
  ...
  return <>
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-2"><button className="rounded-xl border bg-white px-3 py-2 text-sm">Astăzi</button><button className="rounded-xl border bg-white px-3 py-2 text-sm">‹</button><button className="rounded-xl border bg-white px-3 py-2 text-sm">›</button></div><DemoButton>+ Adaugă programare</DemoButton></div>
    <Panel className="overflow-x-auto p-0">
      <div className="min-w-[720px]">
        ...
        <div className="relative grid h-[500px] grid-cols-[62px_repeat(5,1fr)]">
          ...
          <div className="absolute left-[calc(62px+2%)] top-12 w-[16%] rounded-lg border-l-4 bg-green-50 p-2 text-xs" style={{ borderColor: accent }}><strong>Maria Popescu</strong><br />Consultație inițială<br /><span className="text-gray-500">09:00 – 10:00</span></div>
          <div className="absolute left-[calc(62px+21%)] top-36 w-[16%] rounded-lg border-l-4 border-amber-400 bg-amber-50 p-2 text-xs"><strong>Andrei Ionescu</strong><br />Control<br /><span className="text-gray-500">11:00 – 11:30</span></div>
          <div className="absolute left-[calc(62px+61%)] top-60 w-[16%] rounded-lg border-l-4 bg-blue-50 p-2 text-xs" style={{ borderColor: accent }}><strong>Elena Dumitru</strong><br />Consultație<br /><span className="text-gray-500">13:30 – 14:30</span></div>
        </div>
      </div>
    </Panel>
  </>
...
function MessagesPreview() {
  return <Panel className="grid min-h-[590px] overflow-hidden p-0 md:grid-cols-[280px_1fr]">
    ...
    <div className="flex flex-col"><div className="border-b p-4"><strong>Maria Popescu</strong><p className="text-xs text-gray-500">Messenger</p></div><div className="flex-1 space-y-4 bg-gray-50 p-5"><div className="max-w-sm rounded-2xl bg-white p-3 text-sm shadow-sm">Bună ziua! Mai este disponibilă programarea de marți?</div><div className="ml-auto max-w-sm rounded-2xl bg-[#14142b] p-3 text-sm text-white">Bună ziua! Da, vă așteptăm marți la ora 09:00.</div><div className="max-w-sm rounded-2xl bg-white p-3 text-sm shadow-sm">Mulțumesc, confirm programarea.</div></div><div className="flex gap-2 border-t p-4"><input disabled placeholder="Scrie un răspuns..." className="flex-1 rounded-xl border px-3" /><DemoButton>Trimite</DemoButton></div></div>
  </Panel>
}

function BookingsPreview({ label }: { label: string }) {
  return <><div className="mb-4 flex justify-end"><DemoButton>+ Adaugă {label.toLowerCase().replace(/ări$/, 'are')}</DemoButton></div><Panel className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="text-gray-500"><tr><th className="pb-3">Ora</th><th>Client</th><th>Serviciu</th><th>Canal</th><th>Status</th></tr></thead><tbody>{SAMPLE_BOOKINGS.map((b,i)=><tr key={b.time} className="border-t"><td className="py-4">{b.time}</td><td className="font-medium">{b.name}</td><td>{b.service}</td><td>{i%2?'WhatsApp':'Site'}</td><td><Status label={b.status} tone={b.tone} /></td></tr>)}</tbody></table></Panel></>
}

function CustomersPreview({ label, isClinic }: { label: string; isClinic: boolean }) {
  return <><div className="mb-4 flex justify-end"><DemoButton>+ Adaugă {isClinic ? 'pacient' : 'client'}</DemoButton></div><Panel><div className="mb-4 grid gap-3 sm:grid-cols-3"><input disabled placeholder={`Caută în ${label.toLowerCase()}...`} className="rounded-xl border px-3 py-2 sm:col-span-2" /><div className="rounded-xl bg-green-50 p-2 text-center text-sm text-green-700">128 {label.toLowerCase()} activi</div></div>{['Maria Popescu','Andrei Ionescu','Elena Dumitru','Radu Matei'].map((n,i)=><div key={n} className="grid grid-cols-[1fr_auto] gap-3 border-t py-4 sm:grid-cols-3"><div><strong className="text-sm">{n}</strong><p className="text-xs text-gray-500">07•• ••• {42+i}</p></div><span className="hidden text-sm text-gray-500 sm:block">{2+i} programări</span><span className="text-xs text-gray-400">Activ</span></div>)}</Panel></>
}

function ServicesPreview({ label, isVenue }: { label: string; isVenue: boolean }) {
  ...
  return <><div className="mb-4 flex justify-end"><DemoButton>+ Adaugă {isVenue ? 'sală' : 'serviciu'}</DemoButton></div><div className="grid gap-4 md:grid-cols-3">{items.map(([name,duration,price])=><Panel key={name}><div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-green-50" style={{color:'var(--demo-accent)'}}><Tag size={19}/></div><h2 className="font-semibold">{name}</h2><p className="mt-2 text-sm text-gray-500">{duration}</p><p className="mt-4 text-lg font-semibold">{price}</p><span className="mt-4 inline-block rounded-full bg-green-100 px-2 py-1 text-xs text-green-700">Activ</span></Panel>)}</div></>
}
...
function SettingsPreview({ name, teamSize }: { name: string; teamSize: number }) {
  return <div className="max-w-3xl space-y-4"><Panel><h2 className="mb-4 font-semibold">Profil business</h2><label className="text-sm text-gray-500">Nume</label><input disabled value={name} className="mt-1 w-full rounded-xl border bg-gray-50 px-3 py-2.5" /><label className="mt-4 block text-sm text-gray-500">Mod de lucru</label><input disabled value={teamSize > 1 ? 'Echipă' : 'Individual'} className="mt-1 w-full rounded-xl border bg-gray-50 px-3 py-2.5" /></Panel><Panel><h2 className="mb-2 font-semibold">Integrări</h2><div className="flex items-center justify-between border-b py-3"><span>Google Calendar</span><Status label="Conectat" tone="green" /></div><div className="flex items-center justify-between py-3"><span>Mesaje automate</span><Status label="Activ" tone="green" /></div></Panel><DemoButton>Salvează modificările</DemoButton></div>
}
