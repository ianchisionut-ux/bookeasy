...
function optionsFor(dateValue: string, workingHours: WorkingRange[], durationMinutes: number) {
  if (!dateValue) return []
...
export function WorkingDateTimePicker({ value, onChange, workingHours, durationMinutes = 30, minDate }: {
  value: string
  ...
  const times = optionsFor(date, workingHours, durationMinutes)
  return <div>
    <div className="grid grid-cols-[1fr_120px] gap-2">
      <input type="date" value={date} min={minDate} onChange={(event) => {
        const nextTimes = optionsFor(event.target.value, workingHours, durationMinutes)
